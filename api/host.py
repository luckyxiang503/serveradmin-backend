...
@host.get("/allhost", summary="所有主机", response_model=List[HostBase])
async def all_hosts():
    hosts = get_all_hosts()
    if hosts is None:
        raise HTTPException(status_code=404, detail="主机不存在")
...
@host.post("/host", summary='添加主机', response_model=Response200)
async def host_create(host: Host):
    ...
    if add_host(host):
        ...
        raise HTTPException(status_code=400, detail="主机添加失败")


@host.put("/host", summary="修改信息", response_model=Response200)
...
@host.delete("/host", summary="删除主机", response_model=Response200)
